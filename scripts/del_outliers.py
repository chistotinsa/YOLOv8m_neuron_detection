...
def del_outliers(list_annotations: List[List[float]]) -> List[List[float]]:
    ...
    :param list_annotations: List of lists with coordinates of bboxes with detected neurons in xyxy format
    """
    ...
    # Check for standard deviation being zero to avoid division by zero
    if std_x == 0:
        std_x = 0.000001
    if std_y == 0:
        std_y = 0.000001
